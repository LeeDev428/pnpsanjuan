...
def profile():
    ...
    if request.method == 'POST':
        ...
        cursor.execute('SELECT id FROM employee_profiles WHERE user_id = %s', (session['user_id'],))
        ...
        if existing:
            ...
            cursor.execute('''INSERT INTO employee_profiles 
                (user_id, first_name, middle_name, last_name, suffix, unit, station, address,
                 home_address, gender, date_of_birth, place_of_birth, religion,
                 emergency_contact_name, emergency_relationship, emergency_contact_number, rank)
